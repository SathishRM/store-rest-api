...
class Item(Resource):
    ...
    def get(self, name):
        ...
        if item:
            return item.json()
        return {'message': 'Item-{} has not found'.format(name)}, 404
    ...
    def delete(self, name):
        ...
        if item:
            item.delete_from_db()
            return {'message': 'Item-{} has been removed'.format(name)}
        return {'message': 'Item-{} has not found'.format(name)}, 404
    ...
    def put(self, name):
        ...
        try:
            ...
            return {'message': 'An error occurred while inserting/updating the item'}, 500
        return item.json()
